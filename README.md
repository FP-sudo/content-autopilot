...
```
[5/8] Quality gate:
      note: 68/100 → auto-improving density + hook...
        fix: コンテンツ量が不足しています
        fix: フック文を40文字以内に短縮する
      note: 82/100 ✓ (improved +14)
```

**所要時間**: 約2-3分。**人間の介入**: ゼロ。WebSearch失敗→27パターンのフォールバック。品質不合格→自動改善（最大2ラウンド）。

> テーマ「一度命じたら、あとは任せろ」— 7つの判断をシステムが自律的に行います。
