...
Content Autopilotは上の5つの問題を**Pythonスクリプトで自動化**します:
- トピック選定 → WebSearch + ファネル分析で自動決定
- AI臭 → 10パターン検出して自動除去
- マルチプラットフォーム → note・X・IGを1回で同時生成
- 品質チェック → 6軸の定量採点（毎回同じ基準）
- 履歴管理 → JSONに永続保存、次回に反映

`/daily-autopilot` の1コマンドで、全て自律的に実行。
